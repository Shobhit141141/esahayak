...
export const LeadForm = forwardRef<LeadFormRef, LeadFormProps>(
  (
    { initialData, mode = "create", onSave }: LeadFormProps,
    ref: React.Ref<LeadFormRef>
  ) => {
    ...
    const onValidationErrors = (errors: any) => {
      ...
      toast.error(
        "There are validation errors in the form. Please check the console."
      );
    };
    ...
    const autofillForm = () => {
      ...
      const bhk =
        propertyType === "Apartment" || propertyType === "Villa"
          ? faker.helpers.arrayElement(
              BHK_OPTIONS.map((option) => option.value)
            )
          : "";
      ...
      reset({
        ...
        tags: ["urgent", "follow-up", "premium", "budget", "investor"].filter(
    ...
    return (
      <div
        className={`shadow-sm rounded-md bg-transparent w-full max-w-2xl mx-auto ${
          mode === "create" ? "pt-20 p-6 " : ""
        }`}
      >
        <Title order={2} mb="md">
              ...
              {mode === "create" && (
                <div className="flex flex-col items-start gap-2">
                  <Button
                    ...
                    leftSection={<MdAutoFixNormal size={16} />}
                  >
                    Autofill
                  </Button>
                </div>
